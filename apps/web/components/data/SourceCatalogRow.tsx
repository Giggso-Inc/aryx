...
export function SourceCatalogRow({ source, busy, onView, onDownload, onDelete }: {
  ...
  return <article className="border-t border-navy-100 bg-white px-4 py-4 first:border-t-0 xl:grid xl:grid-cols-[minmax(260px,1.6fr)_minmax(135px,.8fr)_120px_110px_110px_130px] xl:items-center xl:gap-3 xl:px-5">
    ...
    <Field label="Total Entities"><Count value={source.total_entities} /></Field>
...
function Count({ value }: { value: number }) {
  const compact = value >= 100_000 ? new Intl.NumberFormat(undefined, { notation: "compact", maximumFractionDigits: 1 }).format(value) : value.toLocaleString();
  return <span className="text-sm font-semibold text-navy-900" title={value.toLocaleString()}>{compact}</span>;
}
