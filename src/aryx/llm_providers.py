...
def _oci_chat_raw(
    ...
    logger.info("[step 8/8] oci_chat  model=%s prompt_chars=%d", spec.name, len(system) + len(user))
    ...
    out_tok = len(text) // 4
    logger.info("[step 8/8] oci_chat ok  response_chars=%d in_tok=%d out_tok=%d", len(text), in_tok, out_tok)
