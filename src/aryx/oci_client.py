...
def _get_auth() -> Any:
    ...
    Auth resolution order:
      1. Env-var config      — ARYX_OCI_USER_OCID set (explicit creds always win)
      2. Instance principal  — running inside OCI with no explicit creds configured
      3. ~/.oci/config       — local dev fallback; profile from OCI_CONFIG_PROFILE

    Explicit env-var credentials take priority over instance principal so that
    containerised deployments on OCI hosts use the configured API key rather than
    the host's instance principal (which typically lacks GenAI policies).
    """
    global _signer
